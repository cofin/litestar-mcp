"""Integration coverage for the Advanced Alchemy JWT-scoped notes examples.
...
def _load_create_app(*, dishka: bool) -> Any:
    module_name = (
        ...
        if dishka
        else "docs.examples.notes.advanced_alchemy.jwt_auth"
    )
...
def test_notes_jwt_rejects_unauthenticated(tmp_path: Path, dishka: bool) -> None:
    ...
    app = create_app(
        database_path=str(tmp_path / f"notes-jwt-{dishka}.sqlite"),
...
def test_notes_jwt_scopes_notes_by_sub(tmp_path: Path, dishka: bool) -> None:
    ...
    create_app = _load_create_app(dishka=dishka)
    app = create_app(
        database_path=str(tmp_path / f"notes-jwt-scoped-{dishka}.sqlite"),
        token_secret=JWT_TEST_SECRET,
    )
    ...
    with TestClient(app=app) as client:
        ...
        items = parse_tool_payload(listed)["items"]
...
def test_notes_jwt_isolates_notes_across_subs(tmp_path: Path, dishka: bool) -> None:
    ...
    create_app = _load_create_app(dishka=dishka)
    app = create_app(
        database_path=str(tmp_path / f"notes-jwt-isolated-{dishka}.sqlite"),
        token_secret=JWT_TEST_SECRET,
    )
    ...
    with TestClient(app=app) as client:
        ...
        bob_items = parse_tool_payload(bob_listed)["items"]
        ...
        bob_delete_payload = bob_delete.get("result")
        if bob_delete_payload is not None:
            content = bob_delete_payload.get("content", [])
            if content:
                import json
        ...
        alice_items = parse_tool_payload(alice_listed)["items"]

...
from litestar_mcp.mcp.auth import DefaultJWKSCache, JWKSCache
...
async def test_default_cache_ttl_expiry(monkeypatch: "pytest.MonkeyPatch") -> "None":
    cache = DefaultJWKSCache()
    now = [100.0]
    monkeypatch.setattr("litestar_mcp.mcp.auth.oidc.monotonic", lambda: now[0])
...
async def test_reset_default_cache_clears_state() -> "None":
    from litestar_mcp.mcp.auth.oidc import get_default_cache, reset_default_cache
...
async def test_injected_cache_consulted_before_fetch(monkeypatch: "pytest.MonkeyPatch") -> "None":
    ...
    from litestar_mcp.mcp.auth.oidc import _validate_oidc_bearer
    ...
    monkeypatch.setattr("litestar_mcp.mcp.auth.oidc._fetch_json_document", fail_fetch)

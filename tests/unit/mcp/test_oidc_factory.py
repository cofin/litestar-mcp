...
shared ``_validate_oidc_bearer`` core in :mod:`litestar_mcp.mcp.auth` so the
...
import litestar_mcp.mcp.auth.oidc as mcp_auth
from litestar_mcp import TokenValidator, create_oidc_validator
from litestar_mcp.mcp.auth.oidc import reset_default_cache
...
async def test_factory_returns_claims_for_valid_token() -> "None":
    ...
    with patch("litestar_mcp.mcp.auth.oidc._fetch_json_document", side_effect=fake_fetch):
...
async def test_factory_jwks_cache_ttl_respected(monkeypatch: "pytest.MonkeyPatch") -> "None":
    ...
    monkeypatch.setattr("litestar_mcp.mcp.auth.oidc.monotonic", fake_monotonic)

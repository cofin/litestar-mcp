...
callers wire it directly into :func:`~litestar_mcp.mcp.auth.create_oidc_validator`
...
from litestar_mcp.mcp.auth import oidc as oidc_internals
from litestar_mcp.mcp.auth.oidc import _validate_oidc_bearer, reset_default_cache
...
async def test_hook_exceptions_are_swallowed(caplog: "pytest.LogCaptureFixture") -> "None":
    ...
    with (
        caplog.at_level(logging.ERROR, logger="litestar_mcp.mcp.auth._oidc"),
...
async def test_factory_forwards_hook() -> "None":
    ...
    from litestar_mcp.mcp.auth import create_oidc_validator

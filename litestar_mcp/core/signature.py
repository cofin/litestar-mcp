"""Helpers for Litestar handler argument reflection and parameter advertisement."""
...
def get_handler_function(handler: Any) -> Callable[..., Any]:
    """Extract the underlying callable from a route handler or callable."""
    if hasattr(handler, "fn"):
        fn = handler.fn
        resolved = getattr(fn, "value", fn)
        candidate = getattr(resolved, "__dishka_orig_func__", resolved)
        if callable(candidate):
            return cast("Callable[..., Any]", candidate)
        return cast("Callable[..., Any]", fn)
    if callable(handler):
        return cast("Callable[..., Any]", handler)
    return cast("Callable[..., Any]", handler.fn)


@dataclass(frozen=True, slots=True)
class AdvertisedHandlerParameter:
    """Handler parameter metadata relevant to protocol callers."""
...
def get_advertised_handler_parameters(
    ...
    *,
    ...
    """Return handler parameters that callers can supply."""
    try:
...
def extract_advertised_handler_arguments(
    ...
    *,
    ...
    """Return prompt-style argument dicts for a Litestar handler."""
...
def iter_dependency_input_parameters(
    ...
    *,
    ...
    try:
        ...
        _logger.warning(
            "Failed to resolve dependencies for handler %r; provider params will be omitted from schema: %s",
            handler_name,
            exc,
        )
        return []
